...
;(function(jQuery, window, document, undefined) {
	...
	Navigation.prototype.initialize = function() {
		...
		this._controls.jQueryabsolute.on('click', 'button', jQuery.proxy(function(e) {
		...
		/*jQueryel.on('focusin', function() {
			jQuery(document).off(".carousel");

			jQuery(document).on('keydown.carousel', function(e) {
				if(e.keyCode == 37) {
					jQueryel.trigger('prev.owl')
				}
				if(e.keyCode == 39) {
					jQueryel.trigger('next.owl')
				}
			});
		});*/
	...
	Navigation.prototype.destroy = function() {
		...
		for (control in this._controls) {
			if (control === 'jQueryrelative' && settings.navContainer) {
